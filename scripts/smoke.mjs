...
try {
  ...
  assert.ok(html.includes("卜卦·投稿时间"));
  ...
  const portals = renderToString(
    createElement(SubmissionPortalList, {
      papers: [
        sample,
        { ...sample, id: "archived", title: "已归档稿件", status: "rejected", url: "" },
        { ...sample, id: "missing", title: "缺少地址", url: "" },
        { ...sample, id: "bad", title: "无效地址", url: "javascript:alert(1)" },
      ],
      onEdit: () => {},
    }),
  );
  assert.ok(portals.includes("在审稿件") && !portals.includes("已归档稿件"));
  assert.ok(portals.includes("选择展示其余稿件"));
  ...
  const progress = renderToString(
    createElement(Progress, {
      data: {
        ...
        status: "revision",
        events: [
          { date: "2026-01-01", status: "submitted" },
          { date: "2026-02-01", status: "revision" },
        ],
      },
    }),
  );
